 ...
 *  GLOBAL DATA
 *********************************************************************************************************************/
extern const str_timer_config_t_ str_timer_config [NUMBER_OF_TIMERS_USED];
...
enu_timer_error_t_ timer_init( void )
{
	...
	for (;TRUE == (uint8_counter < NUMBER_OF_TIMERS_USED); uint8_counter++)
	{
		...
		{
			case TIMER_0:
			switch(str_timer_config[uint8_counter].enu_waveformUsed)
			{
				case TIMER_OV:
...
enu_timer_error_t_ timer_set_time(enu_timer_number_t_ enu_timer_used, f32_t_ f32_desired_time)
{
	...
	{
		case TIMER_0:
		switch(str_timer_config[enu_timer_used].enu_waveformUsed)
		{
			case TIMER_OV:
			uint32_tickTime = str_timer_config[enu_timer_used].enu_prescalerUsed / XTAL_FREQ;
			...
			if (TRUE == (gl_uint8_timer0_rem_ticks != 0))
			{
		...
		switch(str_timer_config[enu_timer_used].enu_waveformUsed)
		{
			...
			if (TRUE == (gl_uint16_timer1_rem_ticks != 0))
			{
		...
		switch(str_timer_config[enu_timer_used].enu_waveformUsed)
		{
			...
			if (TRUE == (gl_uint8_timer2_rem_ticks != 0))
			{
...
enu_timer_error_t_ timer_resume(enu_timer_number_t_ enu_timer_used)
{
	...
	{
		case TIMER_0:
		switch(str_timer_config[TIMER_0].enu_prescalerUsed)
		{
		...
		case TIMER_1:
		switch(str_timer_config[TIMER_1].enu_prescalerUsed)
		{
			case TIMER_PRESCLNG_1:
